  ...
  while (distance > tol){
    ...
    if (iter %% nsync  == 0){
      for (j in 1:nworkers){
        ...
        #workers[[j]]$x <- new$mu + sqrtm(new$Sigma) %*% solve(sqrtm(old$Sigma), workers[[j]]$x - old$mu)
      }
...
natural2mean <- function(theta, d, returnMuSigma=FALSE){   # Transforms a complete natural parameter to mean parameter for Gaussian Approx
  #Sigma <- -0.5*solve(-2*matrix(theta[(d+1):(d+d^2), ], nrow=d))  # as (-2)^{-1} = -0.5
  Sigma <- solve(-2*matrix(theta[(d+1):(d+d^2), ], nrow=d))
