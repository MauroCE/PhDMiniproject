...
# Random-Walk Metropoli Hastings Implementation
```{r}
...
Plot stuff
```{r}
mus <- data.frame(matrix(c(a$history_mu), ncol=2, byrow=TRUE), 1:npasses)
ggplot(data=mus) + geom_point(aes(x=X1, y=X2, color=as.factor(X1.npasses)))
```
