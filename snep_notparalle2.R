  ...
  for (iter in 1:t){
    ...
    for (j in 1:nworkers){
      ...
      log_tilted <- function(x){
        return(as.double(t(tilted_param) %*% s(x)) + log(like(j, c(x))) / workers[[j]]$beta)
      }   
...
### LIKELIHOOD
like <- function(i, params, isss=5000){    # w is a worker
  # Construct file name containing chunk of data
  target <- paste("genetree1/gtree_file", i, sep="")
