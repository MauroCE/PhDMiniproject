...
#Initialise things:
Niter = 100 #this is the number of sweeps through the loci 
Npara = 3 #log(theta), log(R), log(tf), 

Nloci = 100 #in this example (but the file contains 100)
#check this is not greater than number in simulated set
if(Nloci > nsim_loci)stop("Nloci > nsim_loci")
nisamp = 20
numnit = 10000 #number of IS simulations
Smooth = 20 # Smooth should be a number greater than or equal to 1.
           # if it is 1 then EP brutally approximates the posterior immediately (probably unstably)
           # if it is Nloci then this gives the same as aEP of Barthelme and Dehaene
           # if it is bigger than Nloci it will be even smoother than aEP (giving more and more 
           # weight to previous estimate)
if(Smooth < 1)Smooth = 1
Smoothvec = rep(NA,Niter) 
#**check below is compatible with Niter
Smoothvec[1] = Nloci
for(j in 2:Niter)Smoothvec[j] = Smoothvec[j-1]*0.97
Smoothvec = ifelse(Smoothvec < Nloci/2,Nloci/2,Smoothvec)
Smoothvec = round(Smoothvec)
