                 ...
                 epsilon, pop="stable", tol=1e-6, maxiter=1000)
{
  ...
  cat("Iteration: ", iter, " Theta_Update: ", iter %/% nsync, 
      " Distance: ", distance, " Mu: ", to_exp(theta_posterior, d)$mu, "\n")
  while (distance > tol){
    ...
    for (j in 1:nworkers){
      ...
      workers[[j]]$gamma  <- workers[[j]]$gamma + epsilon*(s(x_new) - moment(workers[[j]]$theta_c + workers[[j]]$lambda, d))
    ...
    if (iter %% nsync == 0){
      ...
      total_delta  <- matrix(0, nrow=(2*d), ncol=1)
      for (j in 1:nworkers){
